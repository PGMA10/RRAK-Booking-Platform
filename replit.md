...
- **API Rate Limiting**: Comprehensive rate limiting using express-rate-limit: General API (100 requests/15 min/IP), Booking creation (10/hour/IP to prevent spam), Waitlist submissions (5/hour/IP). Auth endpoints have separate stricter limits (5 failed attempts/15 min/IP).
- **Security Headers**: Helmet middleware with X-Frame-Options: DENY (prevents clickjacking), X-Content-Type-Options: nosniff, Strict-Transport-Security (HSTS in production), Content-Security-Policy (production only), XSS filter, and referrer policy. HTTPS enforcement with HTTP to HTTPS redirect in production.
