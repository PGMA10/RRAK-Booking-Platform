...
- **Loyalty Rewards Program**: Automatic appreciation discount system where customers earn $150 off for every 3 slots booked at regular price. System tracks slots per user with annual reset on January 1st, applies discounts hierarchically in pricing service (after user fixed-price rules, before campaign rules), and deducts used discounts automatically upon payment. Features customer dashboard widget showing progress toward next reward (X/3 slots), available discount balance, and year-to-date total slots. Admin settings table allows configuration of threshold and discount amount. Discounts stack and persist until used.
